...
3. **Max for Live Bridge** (`M4L_Device/`) *(optional)*: A M4L device that provides deep LOM access for hidden device parameters via OSC over UDP
...
- Control playback, scenes, and transport
- Load instruments, effects, and samples from Ableton's browser
- Add, edit, quantize, and transpose MIDI notes
- Control track mixing (volume, pan, mute, solo, arm, sends)
- Manage return tracks and master track
- Create clip automation for device parameters
- Change tempo and other session parameters
- **With M4L Bridge**: Access hidden/non-automatable device parameters on any Ableton device
...
The M4L bridge extends AbletonMCP with access to **hidden/non-automatable device parameters** on any Ableton instrument or effect (Operator, Wavetable, Simpler, Analog, Drift, etc.).

### Quick Setup

1. Requires Ableton Live **Suite** or **Standard + Max for Live**
2. Create a Max MIDI Effect with this simple patch:
   ```
   [udpreceive 9878] → [js m4l_bridge.js] → [udpsend localhost 9879]
   ```
3. Copy `M4L_Device/m4l_bridge.js` to your device's folder
4. Save and load the device on any track

See [`M4L_Device/README.md`](M4L_Device/README.md) for detailed setup instructions.

### Additional Tools (with M4L Bridge)

| Tool | Description |
|---|---|
| `m4l_status` | Check bridge connection |
| `discover_device_params` | List ALL parameters (hidden + public) |
| `get_device_hidden_parameters` | Get hidden parameter details |
| `set_device_hidden_parameter` | Set any parameter by LOM index |
| `list_instrument_rack_presets` | List saved racks (VST/AU workaround) |
...
- VST/AU plugins cannot be loaded directly (API limitation) — save as Instrument Rack first
- Always save your work before extensive experimentation
- See [`WhatItCanDoAndWhatItCant.txt`](WhatItCanDoAndWhatItCant.txt) for a full capabilities reference
